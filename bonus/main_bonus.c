...
bool	init_data(t_args args, t_data *data)
{
	...
	if (!data->philosophers)
		return (ft_write_error("malloc failed for philosophers array\n"),
			false);
...
int	main(int argc, char **argv)
{
	...
	if (!philos)
		return (free(data.philosophers), ft_write_error("malloc failed\n"), 1);
